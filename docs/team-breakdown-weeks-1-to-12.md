# 🏏 IPL Auction Software — 5-Member Team Roadmap & Deliverables (Weeks 1 – 12)

---

## 🧭 Executive Overview
The **IPL Auction Software** is an enterprise-grade full-stack real-time bidding portal built with **Java 17, Spring Boot 3.x, MySQL 8.0, Spring Data JPA, WebSockets (STOMP), Micrometer Actuator Observability, Docker**, and a modern responsive **SPA Frontend**.

The project development is divided cleanly across **5 specialized roles** working in parallel feature branches to ensure complete separation of concerns, strict domain boundaries, and zero merge collisions.
...
| :--- | :--- | :--- | :--- |
| **Member 1: Project Lead & Core Backend** | Architecture & Core REST / Real-Time | Spring Boot base, Team & Player CRUD REST endpoints, Purse deduction rules, Squad size constraints, Global Exception Handler, RTM Engine, Accelerated Round, Compliance Audits, STOMP WebSockets, Auto-Hammer Countdown Timer, Dockerization | `feature/member-1-core-backend` |
| **Member 2: Security & Auth Specialist** | Security & RBAC | Spring Security 6, JWT Token Generation/Validation, User Registration & Login REST APIs, Role-based endpoint guards (`ADMIN` vs `TEAM_OWNER`), WebSocket STOMP handshake JWT validation, Rate limiting | `feature/security-auth` |
| **Member 3: Database & Bidding Engine** | JPA, Concurrency & High Throughput | MySQL DDL `schema.sql`, JPA Entities, Dynamic IPL bid increments, Pessimistic Row Locking (`SELECT ... FOR UPDATE`), Double-bid race condition prevention, Transaction boundaries, Database tuning | `feature/database-bidding` |
| **Member 4: Frontend UI & Real-Time Portal** | SPA & Real-Time Client UI | Responsive SPA UI, Glassmorphism & Neon theme, Auth Screens, Admin Onboarding Console, Live Auction Room, Squad & Purse Tracker, Real-time WebSocket bid ticker, 30s Animated Countdown Ring | `feature/frontend-ui` |
| **Member 5: QA, Testing & DevOps Automation** | QA, Stress Testing & CI/CD | OpenAPI / Swagger UI 3, Postman Collections, JUnit 5 & Mockito Unit/Integration Tests, Concurrency stress tests, Full 12-week test suite, Docker Compose smoke tests, GitHub Actions CI Pipeline | `feature/qa-testing-docs` |

---

# 📅 Week-by-Week Deliverables Breakdown (Weeks 1 to 12)
...
* **In Scope**: Spring Boot project architecture, Maven build configuration, Team CRUD REST APIs, Player CRUD REST APIs, Purse deduction logic, Squad constraints (18–25 players, max 8 overseas), `@RestControllerAdvice` Global Exception Handler, Right To Match (RTM) Engine, Accelerated Round, Compliance Audits, Macro Reports, STOMP WebSockets, 30s Countdown Timer with Auto-Hammer Strike, Actuator Metrics, Multi-stage Docker setup.
...
### 🗓️ Weekly Milestones (Weeks 1 – 12)
...
* **Week 10 (RTM Engine, Accelerated Phase, BCCI Compliance & Macro Analytics)**:
  * Built `RtmService` & `RtmController` with card limits (max 2), live bid matching, purse adjustments, and wallet refunds.
  * Built `AcceleratedAuctionService` & `AcceleratedAuctionController` for unsold shortlist nominations and podium staging.
  * Built `ComplianceAuditService` & `ComplianceController` for BCCI 18–25 roster limits, foreign quotas, and 75% purse spend checks.
  * Built `AuctionReportService` & `ReportController` for whole-auction spend statistics and franchise squad roster exports.
* **Week 11 (Real-Time WebSockets & Automated Countdown Timer)**:
  * Integrated `spring-boot-starter-websocket` and configured STOMP broker (`/topic`, `/app`, `/ws-auction`).
  * Built `LiveAuctionWebSocketController` handling `@MessageMapping("/bid")` and broadcasting to `/topic/bids`.
  * Enabled `@EnableScheduling` and built `AuctionTimerService` (30s ticker emitted to `/topic/timer`, resets on incoming bid).
  * Implemented **Automatic Hammer Strike**: automatically triggers `hammerSold` or `hammerUnsold` when the countdown hits zero.
  * Exposed timer management endpoints (`/api/v1/auction/timer/start`, `/pause`, `/resume`, `/stop`, `/status`).
* **Week 12 (Observability, Rate Limiting, Containerization & Final Release)**:
  * Integrated `spring-boot-starter-actuator` and created Micrometer counters (`auction.bids.total`, `auction.players.sold`).
  * Implemented `RateLimitingFilter` for burst DoS protection on bidding endpoints (10 req/sec limit).
  * Built multi-stage `Dockerfile` and `docker-compose.yml` orchestrating MySQL 8.0 and Spring Boot with health checks.
  * Finalized comprehensive documentation across all 12 weeks.
...
### 🎯 Role Scope & Strict Boundaries
* **In Scope**: Spring Security 6 configuration, Password encoder (`BCryptPasswordEncoder`), JWT utilities (`JwtUtils`/`TokenUtil`), JWT request authentication filter (`JwtAuthenticationFilter`), User registration & Login REST APIs (`/api/v1/auth/register`, `/api/v1/auth/login`), Role-Based Access Control (`ADMIN`, `TEAM_OWNER`), STOMP WebSocket security interceptor.
...
* **Week 10 (Advanced Endpoint Security Alignment)**: Configure role access guards for RTM endpoints, accelerated round permissions, compliance audits, and analytics exports.
* **Week 11 (STOMP Channel Authentication & Destination Guarding)**: Enforce authenticated user verification on `/app/bid` destination; prevent unauthenticated socket connections.
* **Week 12 (Production Security Review & Environment Variable Sanitization)**: Transition JWT secret keys, token expiration timeouts, and admin seeds to environment variable configs (`.env`).
...
* **Week 10 (Transaction Isolation & Multi-Entity Rollbacks)**: Enforce `READ_COMMITTED` transaction boundaries across RTM transfers, ensuring atomic refunds and debits without lock deadlocks.
* **Week 11 (High-Throughput Lock Contention Tuning)**: Minimize lock hold time inside `@Transactional` bidding methods to ensure maximum transactions per second during rapid-fire live bidding.
* **Week 12 (Production Schema Hardening & Migration Scripts)**: Finalize Flyway / DDL migration files, optimize foreign key cascades, and verify data consistency under containerized MySQL 8.0.
...
### 🎯 Role Scope & Strict Boundaries
* **In Scope**: Frontend SPA architecture, responsive CSS theme (glassmorphism, neon styling, variables), Auth UI (Login/Register), Admin Onboarding Console, Live Auction Room UI (Player card, dynamic increment controls, active bidder), Team Squad & Purse Tracker dashboard, STOMP real-time client.
...
* **Week 10 (RTM Notification & Accelerated Round UI)**: Build interactive RTM prompt modal on live hammer drop, accelerated shortlist submission checkboxes, compliance audit scorecard, and printable squad roster export view.
* **Week 11 (Real-Time WebSocket Client & Animated Timer Ring)**: Connect SockJS/STOMP client to `/ws-auction`, subscribe to `/topic/bids` and `/topic/timer`, update live bid ticker with flash animations, and bind 30s countdown to circular SVG timer.
* **Week 12 (Cross-Browser Polish, Mobile Optimization & Production Bundling)**: Verify responsive layouts across desktop, tablet, and mobile; test WebSocket automatic reconnect logic; package static assets.

---

## 🔷 Member 5: QA, Testing & DevOps Automation Lead
...
* **Week 10 (Advanced Rules Test Suite — RTM & Compliance)**: Implement `Week10ServicesTest` covering RTM card limits, atomic transfers, accelerated nominations, BCCI regulatory roster audits, and macro reporting exports.
* **Week 11 (WebSocket & Timer Integration Test Suite)**: Implement `Week11And12IntegrationTest` asserting WebSocket messaging template initialization, 30s countdown state progression, timer reset on bid, and pause/resume lifecycle.
* **Week 12 (Full Regression, Docker Smoke Tests & CI/CD Delivery)**: Execute full 24-test suite validation (`mvn clean test`), verify Docker container orchestration and database healthcheck in `docker-compose.yml`, export Postman JSON collection.
...
# 2. Run complete 12-week test suite (24 tests: Unit + Concurrency + RTM + WebSockets/Timer)
./mvnw.cmd test

# 3. Launch Spring Boot application locally
./mvnw.cmd spring-boot:run

# 4. Launch multi-container environment via Docker Compose
